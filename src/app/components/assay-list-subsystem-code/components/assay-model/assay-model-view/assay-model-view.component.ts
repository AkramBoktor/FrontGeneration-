...
@Component({
  selector: 'app-assay-model-view',
  templateUrl: './assay-model-view.component.html',
  styleUrls: ['./assay-model-view.component.scss'],
  providers: []
})

export class AssayModelViewComponent extends AppBaseComponent implements OnInit {
...
modelTypeSelectOptions: MaterialSelectOptions;
  ...
  constructor(injector: Injector,
    ...
    @Optional() public dialogRef: MatDialogRef<AssayModelViewComponent>,
  ...
  code : [this.selectedAssayModel.code],
  name : [this.selectedAssayModel.name],
  modelType : [this.selectedAssayModel.modelType]
      });
  ...
  disableControls() {
    for (const control of Object.keys(this.assayModelForm.controls)) {
      this.assayModelForm.controls[control].disable();
    }
  }
