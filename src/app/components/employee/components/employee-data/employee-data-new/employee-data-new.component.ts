...
@Component({
  selector: 'app-employee-data-new',
  templateUrl: './employee-data-new.component.html',
  styleUrls: ['./employee-data-new.component.scss'],
  providers: [
    ]
})

export class EmployeeDataNewComponent extends AppBaseComponent implements OnInit {
...
contractAmountIsVisible: boolean;
  ...
  constructor(injector: Injector,
    @Optional() public dialogRef: MatDialogRef<EmployeeDataNewComponent>,
  ...
  iDType : [this.selectedEmployeeData.iDType, [ Validators.required ]]
	...
	this.employeeDataForm.get('appointmentType').valueChanges
	.subscribe(val => {
	if (val === 2) {
	  this.employeeDataForm.get('approvalNumber').setValidators([]);
	  this.approvalNumberIsVisible = true;
	  this.employeeDataForm.get('approvalDate').setValidators([]);
	  this.approvalDateIsVisible = true;
	  this.employeeDataForm.get('contractStartingDate').setValidators([]);
	  this.contractStartingDateIsVisible = true;
	  this.employeeDataForm.get('contractEndDate').setValidators([]);
	  this.contractEndDateIsVisible = true;
	  this.employeeDataForm.get('durationPeriod').setValidators([]);
	  this.durationPeriodIsVisible = true;
	  this.employeeDataForm.get('contractAmount').setValidators([]);
	  this.contractAmountIsVisible = true;
	  this.employeeDataForm.get('approvalNumber').setValidators([]);
	  this.approvalNumberIsVisible = true;
	  this.employeeDataForm.get('contractAmount').setValidators([]);
	  this.contractAmountIsVisible = true;
	}else {
	  this.employeeDataForm.get('approvalNumber').clearValidators();
	  this.approvalNumberIsVisible = false;

	  this.employeeDataForm.get('approvalDate').clearValidators();
	  this.approvalDateIsVisible = false;

	  this.employeeDataForm.get('contractStartingDate').clearValidators();
	  this.contractStartingDateIsVisible = false;

	  this.employeeDataForm.get('contractEndDate').clearValidators();
	  this.contractEndDateIsVisible = false;

	  this.employeeDataForm.get('durationPeriod').clearValidators();
	  this.durationPeriodIsVisible = false;

	  this.employeeDataForm.get('contractAmount').clearValidators();
	  this.contractAmountIsVisible = false;
	...
	this.employeeDataForm.get('approvalNumber').updateValueAndValidity();
	this.employeeDataForm.get('approvalDate').updateValueAndValidity();
	this.employeeDataForm.get('contractStartingDate').updateValueAndValidity();
	this.employeeDataForm.get('contractEndDate').updateValueAndValidity();
	this.employeeDataForm.get('durationPeriod').updateValueAndValidity();
	this.employeeDataForm.get('contractAmount').updateValueAndValidity();
	this.employeeDataForm.get('approvalNumber').updateValueAndValidity();
	this.employeeDataForm.get('contractAmount').updateValueAndValidity();
	});    
  ...
  onSubmit() {
    this.employeeDataService.create(this.employeeDataForm.value)
        .pipe(switchMap(x => {
